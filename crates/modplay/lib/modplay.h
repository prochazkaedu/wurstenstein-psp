...
typedef struct {
	...
	int note; // set to -1 to leave the note intact
	int sample; // set to -1 to leave the sample intact
	int effect_type; // set to -1 to leave the effect type intact
	int effect; // set to -1 to leave the effect intact
} ModActionEventOverride_t;

typedef struct {
	ModActionKind_t kind;

	union {
		ModActionJump_t jump;
		ModActionEventOverride_t event_override;
	};
} ModAction_t;

typedef struct {
	int order; // set to -1 to match on all orders
	int row; // set to -1 to match on all rows
	const ModAction_t *const *actions;
} ModRule_t;
